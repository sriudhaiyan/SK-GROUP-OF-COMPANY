...
export function Planetarium() {
  return (
    ...
    >
      {/* Header */}
      <div className="absolute top-0 left-0 w-full p-4 md:p-6 z-50 flex flex-col md:flex-row items-center justify-between gap-4 pointer-events-none">
        <Link to="/" className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors bg-black/50 px-4 py-2 rounded-full backdrop-blur-sm self-start md:self-auto pointer-events-auto">
          <ArrowLeft className="w-5 h-5" />
          <span className="font-sans tracking-widest text-sm uppercase">Back</span>
        </Link>
        <div className="flex items-center gap-3 md:gap-4 bg-black/50 px-4 md:px-6 py-2 rounded-full backdrop-blur-sm pointer-events-auto">
